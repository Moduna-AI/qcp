...
import type { SafetyReport, QueryResult, QueryMetrics, ApprovalReason } from '../types/index.js';
...
export function printResults(result: QueryResult): void {
  ...
  for (const row of result.rows) {
    table.push(result.fields.map((f) => {
      const val = row[f];
...
export function printMetrics(metrics: QueryMetrics): void {
  ...
  console.log(chalk.dim(`  Tokens in:     `) + chalk.white(String(metrics.tokensIn)));
  console.log(chalk.dim(`  Tokens out:    `) + chalk.white(String(metrics.tokensOut)));
  console.log(chalk.dim(`  SQL gen:       `) + chalk.white(`${metrics.sqlGenerationMs}ms`));
  console.log(chalk.dim(`  Execution:     `) + chalk.white(`${metrics.executionMs}ms`));
  console.log(chalk.dim(`  Summary:       `) + chalk.white(`${metrics.summaryMs}ms`));
  console.log(chalk.dim(`  Total:         `) + chalk.white(`${metrics.totalLatencyMs}ms`));
}
