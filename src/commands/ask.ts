...
import {
  loadConfig, getDatabaseUrl, ensureConfigDir
...
import {
  printQuestion, printSql, printSafetyReport, printResults,
  printSummary, printExplanation, printMetrics, printApprovalWarning,
...
import type {
  DatabaseSchema, LLMProvider, QueryMetrics,
  SqlGenerationResult, SummaryResult
} from '../types/index.js';
...
export async function askCommand(question: string, options: AskOptions = {}): Promise<void> {
  ...
  if (safeMode || options.debug) {
    try {
      const explain = await explainQuery(databaseUrl, safetyReport.processedSql);
  ...
  trackQuery({ provider: config.provider, model: config.model, latencyMs: totalMs });
