...
import { loadConfig, saveConfig, CONFIG_PATH } from '../config/index.js';
import { printSuccess, printError, printSection } from '../output/index.js';
...
export function configShowCommand(): void {
  ...
  console.log(`    Gemini:     ${keyLabel(config.apiKeys.gemini, 'GEMINI_API_KEY')}`);
  console.log(`    OpenAI:     ${keyLabel(config.apiKeys.openai, 'OPENAI_API_KEY')}`);
  console.log(`    Anthropic:  ${keyLabel(config.apiKeys.anthropic, 'ANTHROPIC_API_KEY')}`);
...
export function configSetKeyCommand(provider: string, apiKey: string): void {
  ...
  if (config.provider !== provider) {
    console.log(chalk.dim(`  Tip: switch to this provider with: qcp model set ${provider}`));
  }
}
