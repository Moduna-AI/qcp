...
import { validateSql, getApprovalReasons } from '../safety/index.js';
import { executeQuery } from '../db/index.js';
import { createProvider } from '../llm/index.js';
import {
  printBanner, printSql, printSafetyReport, printResults,
  printSummary, printExplanation, printApprovalWarning,
  printError, printInfo, printSection
} from '../output/index.js';
import {
  initTelemetry, trackActive, trackQuery, trackHumanApproval,
  trackQueryRejected, trackError, shutdownTelemetry
} from '../telemetry/index.js';
import { log } from '../logger/index.js';
import type { DatabaseSchema, LLMProvider } from '../types/index.js';

const HELP_COMMANDS = new Set(['/help', '?', 'help']);
const EXIT_COMMANDS = new Set(['/exit', '/quit', 'exit', 'quit', 'q', ':q', '.exit']);
...
export async function chatCommand(options: ChatOptions = {}): Promise<void> {
  ...
  console.log();
  console.log(
    chalk.bold('  ◆ Query Companion') +
    chalk.dim(' — Interactive Mode')
  );
  console.log(
    chalk.dim(`  Database: `) + chalk.white(schema.databaseName) +
    chalk.dim(` · Provider: `) + chalk.white(`${config.provider}/${config.model}`)
  );
  console.log(
  ...
  while (true) {
    ...
    if (trimmed.startsWith('/')) {
      printInfo(`Unknown command: ${trimmed}. Type /help for available commands.`);
      continue;
    }
...
async function _handleQuestion(
  ...
  try {
    const summaryResult = await session.provider.generateSummary(
      question, safetyReport.processedSql, queryResult
    );
...
function _printHelp(): void {
  ...
  console.log();
  console.log(chalk.dim('  Anything else is treated as a question about your database.'));
  console.log();
...
function _printSchemaOverview(schema: DatabaseSchema): void {
  ...
  for (const table of schema.tables.slice(0, maxShow)) {
    const id = table.schema === 'public' ? table.name : `${table.schema}.${table.name}`;
