...
import { loadConfig, saveConfig, getDatabaseUrl } from '../config/index.js';
import { testConnection, checkReadOnlyUser } from '../db/index.js';
import { printSuccess, printError, printInfo, printWarning } from '../output/index.js';
  ...
  try {
    ...
    if (isReadOnly) {
      ...
      printWarning(
        ...
        chalk.dim('  GRANT SELECT ON ALL TABLES IN SCHEMA public TO qcp_readonly;\n') +
        chalk.dim('  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO qcp_readonly;')
      );
    }
