...
import {
  configExists, saveConfig, ensureConfigDir, createDefaultConfig,
  LOCAL_QCP_DIR, LOCAL_SCHEMA_PATH
} from '../config/index.js';
import { initTelemetry, trackInstall, shutdownTelemetry } from '../telemetry/index.js';
import { printSuccess, printInfo, printWarning, printBanner } from '../output/index.js';
...
export async function initCommand(): Promise<void> {
  ...
  if (!alreadyHasLocal) {
    ...
    printInfo('Run `qcp connect <database-url>` to add your PostgreSQL connection');
  ...
  console.log(chalk.dim('  3. ') + chalk.white('qcp ask "What were our top customers last month?"'));
  console.log();
}
