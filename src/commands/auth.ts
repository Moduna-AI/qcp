...
import ora from 'ora';
import { loadConfig, saveConfig, DEFAULT_MODELS } from '../config/index.js';
import { createProvider } from '../llm/index.js';
import { printSuccess, printError, printInfo, printBanner, printSection } from '../output/index.js';
import { trackProviderSelected } from '../telemetry/index.js';
import type { ProviderName } from '../types/index.js';

const PROVIDER_INFO: Record<ProviderName, { label: string; keyUrl?: string; envVar?: string }> = {
  gemini: {
...
export async function authCommand(): Promise<void> {
  ...
  console.log(chalk.dim('  Configure your AI provider to start querying your database.\n'));
  ...
  if (provider === 'ollama') {
    ...
    const testConfig = { ...config, provider: 'ollama' as ProviderName, model, ollamaHost: host };
  ...
  const existingKey =
    provider === 'gemini'    ? (config.apiKeys.gemini    ?? process.env.GEMINI_API_KEY)    :
    provider === 'openai'    ? (config.apiKeys.openai    ?? process.env.OPENAI_API_KEY)    :
    provider === 'anthropic' ? (config.apiKeys.anthropic ?? process.env.ANTHROPIC_API_KEY) :
    undefined;
  ...
  const { apiKey } = await inquirer.prompt<{ apiKey: string }>([
    {
      ...
      validate: (input: string) => {
        ...
        if (input.trim().length < 10) return 'That doesn\'t look like a valid API key';
        return true;
      },
    },
  ]);
...
export async function authSetKey(provider: string, apiKey: string): Promise<void> {
...
function _printNextSteps(): void {
  ...
  console.log(chalk.dim('  1. ') + chalk.white('qcp connect postgres://user:pass@host/db'));
  console.log(chalk.dim('  2. ') + chalk.white('qcp schema scan'));
  console.log(chalk.dim('  3. ') + chalk.white('qcp ask "What were our top customers?"'));
  console.log();
}
