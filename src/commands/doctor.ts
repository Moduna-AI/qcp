...
import { platform, arch, release, version as nodeVersion } from 'os';
import ora from 'ora';
import chalk from 'chalk';
import {
  loadConfig, getDatabaseUrl, redactConfig,
  QCP_HOME, LOGS_DIR, LOCAL_SCHEMA_PATH, LOCAL_SUPPORT_DIR
} from '../config/index.js';
import { testConnection } from '../db/index.js';
import { loadSchema } from '../schema/index.js';
import { createProvider } from '../llm/index.js';
import {
  printSection, printDoctorCheck, printError, printSuccess, printInfo
} from '../output/index.js';
import {
  initTelemetry, trackDoctor, shutdownTelemetry
} from '../telemetry/index.js';
import { QCP_VERSION } from '../version.js';
import type { DoctorReport, DoctorCheck, HealthStatus } from '../types/index.js';
...
function detectInstallSource(): string {
  ...
  if (mainScript.includes('Homebrew') || execPath.includes('Homebrew')) return 'brew';
...
async function checkDatabase(databaseUrl: string | undefined): Promise<DoctorCheck[]> {
  if (!databaseUrl) {
    return [
      {
  ...
  if (result.connected) {
    checks.push({ name: 'Connected', status: 'healthy', value: result.version });
    checks.push({
  ...
  if (existsSync(LOCAL_SCHEMA_PATH)) {
    try {
      ...
      checks.push({ name: 'Schema', status: 'warning', message: 'Schema file invalid' });
    }
  } else {
    checks.push({
      name: 'Schema',
...
async function checkLLM(config: ReturnType<typeof loadConfig>): Promise<DoctorCheck[]> {
  const checks: DoctorCheck[] = [
  ...
  const hasKey =
    ...
    (config.provider === 'gemini' && !!(config.apiKeys.gemini ?? process.env.GEMINI_API_KEY)) ||
    (config.provider === 'openai' && !!(config.apiKeys.openai ?? process.env.OPENAI_API_KEY)) ||
    (config.provider === 'anthropic' && !!(config.apiKeys.anthropic ?? process.env.ANTHROPIC_API_KEY));

  checks.push({
    name: 'API key',
    status: hasKey ? 'healthy' : 'error',
    value: hasKey ? 'Configured' : undefined,
    message: hasKey ? undefined : `No API key for ${config.provider}. Run: qcp config set-key ${config.provider} YOUR_KEY`,
  });
...
function checkConfiguration(config: ReturnType<typeof loadConfig>): DoctorCheck[] {
  return [
    {
      name: 'Telemetry',
      status: 'healthy',
    ...
    },
    {
      ...
      message: config.safeMode ? undefined : 'Safe mode is off — no approval prompts',
    },
    {
      name: 'Show SQL',
      status: 'healthy',
...
async function generateSupportBundle(
  ...
  try {
    ...
    printInfo('The bundle contains: doctor report, redacted config, recent logs');
    printInfo('No credentials, SQL, schema data, or API keys are included');
    printInfo(`Share this file when reporting issues: ${chalk.cyan('https://github.com/Moduna-AI/qcp/issues')}`);
  } catch (err: unknown) {
