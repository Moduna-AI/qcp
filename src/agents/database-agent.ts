...
export abstract class AbstractDatabaseAgent<TAgentId extends string = string> {
	...
	protected getBaseInstructions(): string[] {
		return [
			...
			"When the user asks to export data, use qcp_export_database_data with either a selected table or a validated read-only SQL query. When the user asks to import data, use qcp_import_database_data and create a new table only.",
			"For imports, never append to, replace, truncate, or alter an existing table. If the target table exists, report the tool refusal and ask for a new destination table.",
			"For imports or exports, use the file format and path stated by the user or the explicit format instruction supplied by the CLI.",
