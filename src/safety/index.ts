...
import type { SafetyReport, ApprovalReason } from '../types/index.js';
...
export function validateSql(sql: string): SafetyReport {
  ...
  if (isExplainStatement(trimmedSql)) {
    ...
    if (!innerReport.safe) {
      report.errors.push(`EXPLAIN inner query rejected: ${innerReport.errors[0]}`);
      return report;
    }
  ...
  try {
    ...
    report.errors.push(`SQL parsing failed: ${msg}. Query rejected for safety.`);
    return report;
  }
  ...
  if (!hasLimit) {
    ...
    if (userLimit !== null && userLimit > MAX_LIMIT) {
      processedSql = processedSql.replace(/\bLIMIT\s+\d+/i, `LIMIT ${MAX_LIMIT}`);
...
export function detectSensitiveTables(sql: string, patterns: string[]): string[] {
